...
def parseData(workingDirectory):
    ...
    for rof in rofFiles:
        ...
        PIDArray = jq('.[] | select(.["af-model"] != "GenericFile") |.pid?, .metadata["dc:source"], .metadata["dc:title"]').transform(metadata, multiple_output=True)
...
def writePIDUpdateFile(PIDArray,rof):
    ...
    f = open(outputFile, 'w')
    f.write('curate_id,filename,dc:title\n')
    while PIDArray != []:
      varPID = '"' + str(PIDArray.pop(0)) + '"'
      varSource = '"' + str(PIDArray.pop(0)) + '"'
      varFilename = varSource[-7:]
      varTitle = '"' + str(PIDArray.pop(0)) + '"'
      row = varPID + ',' + varFilename + ',' + varTitle + '\n'
      f.write(row)
    f.close()
